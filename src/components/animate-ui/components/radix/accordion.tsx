...
function AccordionContent({ className, children, ...props }: AccordionContentProps) {
  // Filter out MotionValueNumber and MotionValueString
  const renderableChildren = React.useMemo(() => {
    if (typeof children === "object" && (children as any)?.constructor?.name?.includes("MotionValue")) {
      return null;
    }
    if (Array.isArray(children)) {
      return children.filter(
        (child) => !(typeof child === "object" && (child as any)?.constructor?.name?.includes("MotionValue"))
      );
    }
    return children;
  }, [children]);
  ...
  return (
    <AccordionContentPrimitive {...props}>
      <div className={cn("text-sm pt-0 pb-4", className)}>{renderableChildren as any}</div>
