...
export const ModernBookCover = ({
  ...
  return (
    <div className={`z-10 group [perspective:800px] w-min [--shadowColor:#111] ${className}`}>
      <style jsx>{`
      ...
      <div
        style={{
          ...
          height: `calc(${sizeMap[size].width} * 1.3333)`,
          animation: isStatic ? "none" : "bookRotate 3s ease-in-out infinite",
        }}
        className={`relative [transform-style:preserve-3d] ${
          isStatic ? "[transform:rotateY(-30deg)]" : ""
        } aspect-3/4 ${radiusMap[radius]}`}
      >
        <div
          className={`absolute inset-y-0 overflow-hidden size-full left-0 text-white flex flex-col justify-end p-6 ${
            coverImage ? "bg-white" : `bg-linear-to-tr ${gradient.from} ${gradient.to}`
          } ${radiusMap[radius]}`}
          style={{
          ...
          {coverImage ? (
            <>
              <Image
                ...
                alt="Book Cover"
                fill
                className={`object-cover ${radiusMap[radius]}`}
                sizes={sizeMap[size].width}
              />
              <div
              ...
              />
              {children && <div className="pl-1 relative z-10">{children}</div>}
            </>
          ) : (
            <>
              <div
        ...
        <div
          className={`absolute inset-y-0 overflow-hidden size-full left-0 flex flex-col justify-end p-6 ${
            coverImage ? "bg-white" : `bg-linear-to-tr ${gradient.from} ${gradient.to}`
          } ${radiusMap[radius]}`}
          style={{
            transform: "translateZ(-25px)",
...
export const BookTitle = ({ children, className = "" }: BookTitleProps) => (
  <h1 className={`font-bold select-none mt-3 mb-1 text-balance ${className}`}>{children}</h1>
);
...
export const BookDescription = ({ children, className = "" }: BookDescriptionProps) => (
  <p className={`opacity-80 select-none text-xs/relaxed ${className}`}>{children}</p>
);
