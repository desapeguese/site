...
export function RadialFlow({ topics, badgeName, centralDotColor = "#FFFFFF" }: RadialFlowProps) {
  ...
  const getLabelPosition = useCallback(
    (position: { x: number; y: number }) => {
      ...
      return {
        x: position.x < 50 ? Math.min(position.x + 5, 20) : Math.max(position.x - 5, 80),
        y: position.y,
      };
    },
    [isMobile]
  ...
  const getPathData = useCallback(
    (topic: Topic) => {
      ...
      const controlX = pos.x < 50 ? x + (centerX - x) * 0.75 : x - (x - centerX) * 0.75;
  ...
  const getTopicLabelClasses = useCallback(
    (topic: Topic) =>
      `absolute transform -translate-x-1/2 -translate-y-1/2 rounded-md 
  ...
  return (
    <div ref={containerRef} className="w-full h-full  relative overflow-hidden min-h-[300px]">
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 mb-8">
        <div className="my-12 relative w-48 h-48">
          ...
          <div className="absolute inset-4 bg-primary rounded-lg -rotate-12 opacity-100"></div>
          <div className="absolute inset-8 flex items-center justify-center">
            <Image
              src={"/img/LOGO_INSTITUTO_BRANCA.png"}
              alt={"Logo da Biomob"}
              aria-label={"Logo da Biomob"}
              width={120}
              height={120}
              quality={100}
            />
      ...
      {topics.map((topic) => (
        <Button key={topic.id} className={getTopicLabelClasses(topic)} style={getLabelStyle(topic)}>
          {topic.name}
        </Button>
      ))}
    </div>
